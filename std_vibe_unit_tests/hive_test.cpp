...
TEST(HiveHeader, HeaderAvailabilityAndFeatureMacro)
{
	// This test documents whether the implementation currently provides <hive>.
#if VIBE_HAS_HIVE
#ifdef __cpp_lib_hive
	EXPECT_GE(__cpp_lib_hive, 0L);
#endif
#else
	GTEST_SKIP() << "<hive> is not available in this standard library.";
#endif
...
namespace {
	...
	TEST(HiveHeader, AvailabilityAndFeatureMacro)
	{
#if VIBE_HAS_HIVE
#ifdef __cpp_lib_hive
		EXPECT_GE(__cpp_lib_hive, 202311L);
#else
		FAIL() << "<hive> exists but __cpp_lib_hive missing.";
#endif
#else
		GTEST_SKIP() << "<hive> unavailable on this toolchain.";
#endif
	}
